...
class TemperatureMeasurement(EnvExperiment):
    ...
    def build(self):
        ...
        self.setattr_argument("freq_probe_scan_mhz",    Scannable(default=LinearScan(65, 85, 5),
                                                                  global_min=60, global_max=110, global_step=1,
                                                                  units="MHz", scale=1, ndecimals=1))
    ...
    def prepare(self):
        ...
        self.freq_probe_ftw = self.dds_probe.frequency_to_ftw(self.freq_probe_mhz * MHz)
        ...
        # set up datasets
        self.set_dataset("temperature_measurement_repump_on", np.zeros([len(self.freq_probe_scan_mu) * self.repetitions, 3]), broadcast=True)
        self.set_dataset("temperature_measurement_repump_off", np.zeros([len(self.freq_probe_scan_mu) * self.repetitions, 3]), broadcast=True)
