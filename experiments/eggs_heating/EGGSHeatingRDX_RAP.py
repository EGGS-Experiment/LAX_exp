...
class EGGSHeatingRDX_RAP(LAXExperiment, Experiment):
    """
    Experiment: EGGS Heating RAP

    Cool the ions to the ground state of motion via sideband cooling,
    then apply bichromatic heating tones, and measure ion temperature via sidebands or RAP
    """
    name = 'EGGS Heating RDX RAP'
    kernel_invariants = {
        # hardware parameters
    ...
    def build_experiment(self):
        ...
        # RAP arguements
        self.setattr_argument("enable_rap", BooleanValue(default=False), group='RAP')
        self.setattr_argument("att_rap_db", NumberValue(default=31.5, precision=1, step=0.5, min=8, max=31.5),
                              group="RAP")
        ...
        self.setattr_argument("freq_rap_center_mhz",
                              NumberValue(default=101.3318, step=0.0100, precision=6, min=60, max=200), group='RAP')
        self.setattr_argument("freq_rap_dev_khz", NumberValue(default=100., step=0.01, precision=2, min=1, max=10000),
                              group='RAP')
        self.setattr_argument("time_rap_us", NumberValue(default=200., min=1, max=100000, step=1, scale=1, precision=5),
                              group="RAP")
    ...
    def prepare_experiment(self):
        ...
        #config time_rap_us into numpy array to make config list the same size regardless of readout method
        self.time_rap_mu_list = np.array([self.time_rap_mu])
    ...
    def initialize_experiment(self) -> TNone:
        ...
        self.rap_subsequence.record_dma()
    ...
    def run_main(self) -> TNone:
        ...
        for trial_num in range(self.repetitions):
            ...
            while _config_iter < self.num_configs:
                ...
                # config_vals[0] is either frequency of sideband readout or rap time
                ...
                if not self.enable_rap:
                    self.qubit.set_mu(config_vals[0], asf=self.sidebandreadout_subsequence.ampl_sideband_readout_asf,
    ...
    def analyze_experiment(self):
        ...
        for ch1_turns in self.phase_eggs_heating_ch1_turns_list:
            ...
            try:
                ...
                if sorting_col_num == 3 or sorting_col_num == 2 and not self.enable_rap:
