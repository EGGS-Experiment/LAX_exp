...
class MicromotionCompensation(ParametricSweep.ParametricSweep, Experiment):
    ...
    def build_experiment(self):
        ...
        # modulation - mode #1
        self.setattr_argument("freq_mod0_khz",                      NumberValue(default=1055, ndecimals=3, step=10, min=1, max=10000), group='modulation_1')
        ...
        # modulation - mode #2
        self.setattr_argument("freq_mod1_khz",                      NumberValue(default=1277, ndecimals=3, step=10, min=1, max=10000), group='modulation_2')
    ...
    def prepare_experiment(self):
        ...
        self.att_mod_mu =                                           att_to_mu(self.att_mod_db * dB)
    ...
    @kernel(flags={"fast-math"})
    def _sweep_voltage_mode(self, mode_freq_ftw: TInt64, mode_num: TInt32,
        ...
        # create local holder dataset
        # todo: make more elegant
        tmp_dataset_holder = np.zeros((self.repetitions_per_voltage * len(voltage_vec_v_arr), 4),
                                           dtype=float)
        tmp_dataset_iter = 0
        self.core.break_realtime()
        ...
        for rep_num in range(self.repetitions_per_voltage):
            ...
            for voltage_vec_v in voltage_vec_v_arr:
                ...
                # todo: see if this first core break realtime is necessary
                # self.core.break_realtime()
                ...
                # with parallel:
                self.core.break_realtime()
                res_tmp = self._process_results(mode_freq_ftw, voltage_mod0_v, voltage_mod1_v, pmt_timestamp_list)

                # also store values in a local dataset
                tmp_dataset_holder[tmp_dataset_iter] = res_tmp
                tmp_dataset_iter += 1
                self.core.reset()

        # do complex linear fit to extract optimal voltage
        opt_voltages_v_list = self._extract_optimum(mode_num, tmp_dataset_iter)
        return opt_voltages_v_list

    @rpc
    def _process_results(self, freq_mu: TInt32,
                         voltage_mod0_v: TFloat, voltage_mod1_v: TFloat,
                         timestamp_mu_list: TArray(TInt64, 1)) -> TArray(TFloat, 1):
        """
        ...
        # todo: come up with better way of returning values
        # return np.array([correlated_ampl, correlated_phase])
        return np.array([voltage_mod0_v, voltage_mod1_v, correlated_ampl, correlated_phase])
