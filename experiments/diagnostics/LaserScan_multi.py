...
class LaserScanMulti(LAXExperiment, Experiment):
    ...
    def build_experiment(self):
        ...
        self.setattr_argument("freq_sideband_1",                      NumberValue(default=0.76, min=0, max=20, step=1,
                                                                        unit="MHz", scale=1, ndecimals=6),group=self.name)
        self.setattr_argument("freq_sideband_2",                      NumberValue(default=1.07, min=0, max=20, step=1,
                                                                        unit="MHz", scale=1, ndecimals=6),group=self.name)
    ...
    def prepare_experiment(self):
        ...
        self.freq_rsb1_scan_ftw =                                   np.array([hz_to_ftw((freq_mhz - self.freq_sideband_1 / 2) * MHz) for freq_mhz in self.freq_qubit_scan_mhz])
        self.freq_rsb2_scan_ftw =                                   np.array([hz_to_ftw((freq_mhz - self.freq_sideband_2 / 2) * MHz) for freq_mhz in self.freq_qubit_scan_mhz])
        ...
        #print(self.freq_qubit_scan_ftw)
