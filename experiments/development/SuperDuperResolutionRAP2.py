...
class SuperDuperResolution(LAXExperiment, Experiment):
    ...
    kernel_invariants = {
        ...
        # tmp rap
        'profile_729_test', 'ampl_rap_asf', 'freq_carr_ftw', 'freq_shelve1_ftw', 'freq_shelve2_ftw',
        'freq_shelve3_ftw', 'time_carr_mu', 'time_shelve1_mu', 'time_shelve2_mu', 'time_shelve3_mu'
        # tmp rap
    }
    ...
    def initialize_experiment(self) -> TNone:
        ...
        # tmp rap
        self.core.break_realtime()
        with self.core_dma.record("_HANDLE_RAP_PREPARE"):
            self.rap_prepare()
        self.core.break_realtime()
        with self.core_dma.record("_HANDLE_RAP_READOUT"):
            self.rap_readout()
        self.core.break_realtime()
        # tmp rap

    def _prepare_rap_stuff(self):
        self.profile_729_test = 3
    ...
    def run_main(self) -> TNone:
        ...
        # tmp rap
        _handle_rap_prepare = self.core_dma.get_handle('_HANDLE_RAP_PREPARE')
        self.core.break_realtime()
        _handle_rap_readout = self.core_dma.get_handle('_HANDLE_RAP_READOUT')
        self.core.break_realtime()
        # tmp rap
        ...
        for trial_num in range(self.repetitions):
            ...
            for config_vals in self.config_experiment_list:
                ...
                delay_mu(256)
                at_mu(now_mu() & ~0x7)
                self.core_dma.playback_handle(_handle_rap_prepare)
                # tmp rap
                ...
                delay_mu(256)
                at_mu(now_mu() & ~0x7)
                self.core_dma.playback_handle(_handle_rap_readout)
                # if self.enable_RAP:
                #     self.qubit.set_att_mu(self.att_rap_mu)
