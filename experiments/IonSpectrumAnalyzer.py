...
class IonSpectrumAnalyzer(SidebandCooling.SidebandCooling):
    """
    ...
    ***todo: redocument***
    ...
    def build_experiment(self):
        ...
        self.setattr_argument("freq_ionSpecAnal_sideband_offset_khz_list",  Scannable(
                                                                                default=[
                                                                                    ExplicitScan([0]),
                                                                                    CenterScan(0, 20, 0.5, randomize=True)
                                                                                ],
    ...
    def prepare_experiment(self):
        ...
        # note: the 5 values are [carrier_freq_hz, sideband_freq_hz, rsb_ampl_frac, bsb_ampl_frac, carrier_ampl_frac]

        # ion specanal - insert
        # note: 6 values are [carrier_freq_hz, sideband_freq_hz, rsb_ampl_frac, bsb_ampl_frac, carrier_ampl_frac, offset_freq_hz]
        # ion specanal - insert
        ...
        self.config_eggs_heating_list[:, [0, 1, 2, -1]] =                   np.stack(np.meshgrid(self.freq_readout_ftw_list,
                                                                                     ...
                                                                                     -1).reshape(-1, 4)
        self.config_eggs_heating_list[:, [2, 3, 4]] =                       np.array([self.ampl_eggs_heating_rsb_pct,
                                                                                      self.ampl_eggs_heating_bsb_pct,
    ...
    def phaser_configure(self, carrier_freq_hz: TFloat, sideband_freq_hz: TFloat, offset_freq_hz: TFloat):
        ...
        # oscillator 1 (BSB)
        self.phase_ch0_osc1 =           (sideband_freq_hz + offset_freq_hz) * self.phaser_eggs.t_sample_mu * ns
        self.phase_ch1_osc1 =           (sideband_freq_hz + offset_freq_hz) * self.phaser_eggs.t_sample_mu * ns
