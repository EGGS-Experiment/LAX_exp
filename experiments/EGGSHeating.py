...
class EGGSHeating(SidebandCooling.SidebandCooling):
    ...
    def build_experiment(self):
        ...
        self.setattr_argument("ampl_eggs_heating_rsb_pct",                  NumberValue(default=40, ndecimals=2, step=10, min=0.01, max=99), group='EGGS_Heating.waveform.ampl')
        self.setattr_argument("ampl_eggs_heating_bsb_pct",                  NumberValue(default=40, ndecimals=2, step=10, min=0.01, max=99), group='EGGS_Heating.waveform.ampl')
    ...
    def phaser_run_psk(self, ampl_rsb_frac: TFloat, ampl_bsb_frac: TFloat, ampl_dd_frac: TFloat):
        ...
        for dd_config_vals in self.config_dynamical_decoupling_psk_list[1:]:
            ...
            with parallel:
                self.phaser_eggs.channel[0].oscillator[2].set_amplitude_phase(amplitude=ampl_dd_frac, phase=self.phase_ch0_osc2 + (dd_config_vals[1] * 0.25), clr=0)
