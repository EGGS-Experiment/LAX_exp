...
class ReservoirEngineering(LAXExperiment, Experiment):
    """
    ...
    kernel_invariants = {
        ...
        # tmp remove - reservoir engineering
        'time_spinpol_re_mu', '_ampl_quench_re_asf', 'profile_re', 'time_spinpolarization_re_mu_list',
        'time_spinpol_interval_re_mu'
        # tmp remove - reservoir engineering
    }
    ...
    def prepare_experiment(self):
        ...
        # tmp remove - reservoir engineering
        self._prepare_reservoir_engineering()
        # tmp remove - reservoir engineering
    ...
    def _prepare_reservoir_engineering(self):
        self.profile_re = 6
        self._ampl_quench_re_asf = self.repump_qubit.amplitude_to_asf(5. / 100.)
        self.time_spinpol_re_mu = self.get_parameter('time_spinpol_us', group='timing', override=True,
                                                     conversion_function=seconds_to_mu, units=us)

        self.time_spinpol_interval_re_mu = self.core.seconds_to_mu(self.sidebandcool_subsequence.time_per_spinpol_us)
        self.time_spinpolarization_re_mu_list = np.arange(
            self.time_spinpol_interval_re_mu,
            self.time_cat1_bichromatic_mu - 10000,
            self.time_spinpol_interval_re_mu + self.time_spinpol_re_mu + 200,
            dtype=np.int64
        )
    ...
    def initialize_experiment(self) -> TNone:
        ...
        # tmp remove - set up 854nm for reservoir engineering
        self.repump_qubit.set_mu(self.repump_qubit.freq_repump_qubit_ftw, asf=self._ampl_quench_re_asf,
                                 profile=self.profile_re, phase_mode=ad9910.PHASE_MODE_CONTINUOUS)
    ...
    def run_main(self) -> TNone:
        ...
        for trial_num in range(self.repetitions):
            ...
            for config_vals in self.config_experiment_list:
                ...
                # tmp remove - reservoir engineering
                num_spinpols_tmp = np.int32(time_cat2_cat_mu // self.time_spinpol_interval_re_mu)
                # tmp remove - reservoir engineering
                self.core.break_realtime()
                ...
                self.qubit.cpld.io_update.pulse_mu(8)
                self.repump_qubit.set_profile(self.profile_re)
                ...
                # first schedule spinpol
                self.spin_polarize(num_spinpols_tmp)
                # go back in time and do reservoir engineering
                at_mu(time_start_mu)
    ...
    @kernel(flags={"fast-math"})
    def spin_polarize(self, num_spinpols: TInt32=1) -> TNone:
        for time_intervals_mu in range(num_spinpols):
            delay_mu(self.time_spinpol_interval_re_mu)
