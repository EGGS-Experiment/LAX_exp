...
class CatStateInterferometerTickleMS(LAXExperiment, Experiment):
    ...
    def _build_arguments_tickle_modes(self):
        ...
        self.setattr_argument("freq_tickle_detuning_mode_khz_list",
                              PYONValue([[-1,1], [-1,1]]),
                              group=_argstr,
    ...
    def prepare_experiment(self):
        ...
        num_detunings = np.shape(self.freq_tickle_detunings_mode_khz_list)[1]
        ...
        self.profiles = [self.profile_729_cat1, self.profile_729_cat2, self.profile_729_ms]
        ...
        self.phase_cat2_offset = self.qubit.phase_to_turns(0.5)
    ...
    def _prepare_experiment_ion_parameters(self):
        ...
        """
        self.freq_carrier_ftw = self.qubit.frequency_to_ftw(self.freq_carrier_mhz)

        self.freq_secular_ftw_list = array([self.dds_pulse_shaper_list[idx].dds_target.frequency_to_ftw(freq_secular_khz_list[idx]*kHz)
    ...
    def _prepare_experiment_ms_gate(self):
        ...
        self.freq_ms_gate_secular_detuning_ftw = self.qubit.frequency_to_ftw(self.freq_ms_secular_detuning_khz)
    ...
    def _prepare_experiment_cat_modes(self):
        ...
        for idx in range(len(self.ampls_cat_asf)):
            self.ampls_cat_asf[idx] = array([self.qubit.singlepass0.amplitude_to_asf(ampl_pct / 100.)
                                    for ampl_pct in self.ampls_cat_pct[idx]])
        ...
        for idx in range(len(self.atts_cat_mode_db)):
            att_reg_cat_interferometer = 0x00000000 | (
                    (self.qubit.att_qubit_mu << ((self.qubit.beam.chip_select - 4) * 8)) |
                    (self.att_dynamical_decoupling_mu << ((self.qubit.singlepass0.chip_select - 4) * 8)) |
                    (att_to_mu(self.atts_cat_db[idx][0] * dB) << ((self.qubit.singlepass1.chip_select - 4) * 8)) |
                    (att_to_mu(self.atts_cat_db[idx][1] * dB) << ((self.qubit.singlepass2.chip_select - 4) * 8))
            )
    ...
    def _prepare_experiment_tickle_modes(self):
        ...
        """
        self.dds_pulse_shaper_list = [0]*len(self.ampl_tickle_modes_pct)
        for idx in range(len(self.ampl_tickle_modes_pct)):
            dds_pulse_shaper = DDSPulseShaper(self, dds_target=self.dds_dipole.dds,
                           ...
                           phase_autoclear=1)
            self.dds_pulse_shaper_list[idx] = dds_pulse_shaper

        self.att_tickle_mu_list = [att_to_mu(att_tickle_modes_db[idx] * dB)
                                   for idx in range(len(self.att_tickle_modes_db))]
        self.phase_tickle_pow_list = array([self.dds_pulse_shaper_list[idx].turns_to_pow(self.phase_tickle_turns)
        for idx in range(len(self.phase_tickle_turns))])


        self.freq_tickle_detuning_ftw_list = np.zeros(np.shape(self.freq_tickle_detuning_mode_khz_list))

        for idx in range(len(self.freq_tickle_detuning_mode_khz_list)):
            modes_detunings_ftw = [self.dds_pulse_shaper_list[idx].dds_target.frequency_to_ftw(freq_tickle_detuning_khz*kHz)
                                         for freq_tickle_detuning_khz in self.freq_tickle_detuning_khz_list[idx]]
    ...
    def _prepare_argument_checks(self) -> TNone:
        ...
        if np.shape(self.ampls_cat_mode_pct) != np.shape(atts_cat_mode1_db):
        ...
        if (np.shape(self.freq_secular_khz_list) != np.shape(self.att_tickle_modes_db)
            or np.shape(self.att_tickle_modes_db) != np.shape(self.ampl_tickle_modes_pct)
            or np.shape(self.ampl_tickle_modes_pct)!= np.shape(self.phase_tickle_turns)):
        ...
        if np.shape(self.freq_tickle_detuning_mode_khz_list)[0] != np.shape(self.ampl_tickle_modes_pct)[0]:
            raise ValueError('Must provide the same number of secular frequency detuning sets as '
                             ...
                             'the tickle pulses for each mode\n'
                             'Please check the "freq_tickle_detuning_mode_khz_list" parameter')
    ...
    def run_main(self) -> TNone:
        ...
        for trial_num in range(self.repetitions):
            for config_vals in self.config_experiment_list:
                ...
                freq_secular_ftw = self.freq_secular_ftw_list[idx_freq_secular]
                freq_tickle_detuning_ftw = self.freq_tickle_detuning_ftw_list[idx_freq_secular][idx_freq_tickle_detuning]

                self.update_profile_configuration(idx_freq_secular)
                '''
                BEGIN MAIN SEQUENCE
                '''
                ...
                dds_pulse_shaper = self.dds_pulse_shaper_list[idx_freq_secular]
                dds_pulse_shaper.sequence_initialize()

                dds_pulse_shaper.dds_target.set_ftw(freq_secular_ftw + freq_tickle_detuning_ftw)
                dds_pulse_shaper.dds_target.set_pow(self.phase_tickle_pow_list[idx_freq_secular])
                dds_pulse_shaper.dds_target.set_att_mu(self.att_tickle_mu_list[idx_freq_secular])
                dds_pulse_shaper.dds_target.sw.off()
                delay_mu(8)

                # set up config of shaped pulses to be fired for tickling
                # also sets up phase autoclear
                dds_pulse_shaper.configure_train(self.time_tickle_mu)
                t0 = now_mu() + 1000
                at_mu(t0)
                dds_pulse_shaper.dds_target.cpld.io_update.pulse_mu(8)
                at_mu(t0)
                self.qubit.io_update()
                # for ururuk channel used for tickling keep RAM enabled but ensure we don't clear phase on io_update
                dds_pulse_shaper.dds_target.set_cfr1(ram_enable=1, phase_autoclear=0,
                ...
                if self.enable_ms_gate:
                    ...
                    self.pulse_bichromatic(self.profile_729_ms, t0,
                                           self.time_ms_gate_mu,
                                           self.phase_ms_dynamical_decoupling_pow)

                '''
                CAT #1
                '''
                self.qubit.cpld.set_all_att_mu(self.atts_cat_reg_mu_list[freq_secular_idx])
                # cat1 - bichromatic cat pulse
                self.pulse_bichromatic(self.profile_729_cat1, t0,
                ...
                if self.enable_dynamical_decoupling:
                    ...
                    self.qubit.on()
                with parallel:
                    dds_pulse_shaper.run_train_single()
                    if self.enable_dynamical_decoupling:
                ...
                CAT #2
                '''
                self.pulse_bichromatic(self.profile_729_cat2, t0,
                                       self.time_cat_bichromatic_mu,
                                        self.phase_cat_dynamical_decoupling_pow)
                ...
                dds_pulse_shaper.sequence_cleanup()
    ...
    @kernel(flags={'fast-math'})
    def pulse_bichromatic(self, profile: TInt32, t0: TInt64,
        ...
        self.qubit.off()
        self.setup_beam_profile(profile, t0)
        ...
        if self.enable_dynamical_decoupling:
            self.set_carrier_phase(phase_dd_pow, profile=profile, t0 = t0)
            self.qubit.singlepass0_on()
    ...
    def set_carrier_phase(self, phase_dd_pow_: TInt32,
                             profile: TInt32, t0: TInt64) -> TNone:
        """
    ...
    @kernel(flags={"fast-math"})
    def setup_beam_profile(self, profile, t0) -> TNone:
        """
    ...
    def set_default_profile_configuration(self):
        ...
        self.phase_beams_pow_list[self.profile_729_cat1][1] = self.phase_cat_dynamical_decoupling_pow

        self.phase_beams_pow_list[self.profile_729_cat2][1] = self.phase_cat_dynamical_decoupling_pow
        ...
        self.freq_beams_ftw_list[self.profile_729_ms][2] = self.qubit.freq_singlepass1_default_ftw - self.freq_secular_ftw_list[0] - self.freq_ms_gate_secular_detuning_ftw
        self.freq_beams_ftw_list[self.profile_729_ms][3] = self.qubit.freq_singlepass2_default_ftw + self.freq_secular_ftw_list[0] + self.freq_ms_gate_secular_detuning_ftw

        self.phase_beams_pow_list[self.profile_729_ms][1] = self.phase_ms_dynamical_decoupling_pow
        self.phase_beams_pow_list[self.profile_729_ms][2] = self.ms_phases[0]
        self.phase_beams_pow_list[self.profile_729_ms][3] = self.ms_phases[1]

    @kernel(flags={"fast-math"})
    def update_profile_configuration(self, secular_freq_idx):
        # set up values consistent across cat profiles
        for profile in [self.profile_729_cat1, self.profile_729_cat2]:
            self.ampl_beams_asf_list[profile][2] = self.ampls_cat_asf_list[secular_freq_idx][0]
            self.ampl_beams_asf_list[profile][3] = self.ampls_cat_asf_list[secular_freq_idx][1]

            self.freq_beams_ftw_list[profile][2] = self.qubit.freq_singlepass1_default_ftw - self.freq_secular_ftw_list[secular_freq_idx]
            self.freq_beams_ftw_list[profile][3] = self.qubit.freq_singlepass2_default_ftw + self.freq_secular_ftw_list[secular_freq_idx]
