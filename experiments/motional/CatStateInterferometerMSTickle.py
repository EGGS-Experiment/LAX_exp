...
class CatStateInterferometerMSTickle(LAXExperiment, Experiment):
    """
    Experiment: Cat State Interferometer MS Tickle
    ...
    """
    name = 'Cat State Inteferometer MS Tickle'
    kernel_invariants = {
    ...
    def build_experiment(self):
        ...
        self._build_arguements_ion_parameters()
    ...
    def _build_arguements_ion_parameters(self):
        _arg_str = "ion_parameters"
        self.setattr_argument("freq_center_mhz_list", Scannable(
            default=[
        ...
        self.setattr_argument("freq_secular_khz_list", Scannable(
            default=[
            ...
            unit="kHz", scale=1, precision=3
                              ...
                              tooltip="Single-sided detuning frequency for the bichromatic pulses, applied via singlepass DDSs.\n"
                                      ...
                                      "i.e. frequencies for [singlepass1, singlepass2] is set as [beams.freq_mhz.freq_singlepass1_mhz - freq_secular_khz, "
                                      "beams.freq_mhz.freq_singlepass2_mhz + freq_secular_khz].")
    ...
    def _build_arguments_ms(self):
        """
        Build arguments for molmer sorenson gate
        """
        # cat #1 config

        _arg_str = "ms"
        self.setattr_argument("enable_ms", BooleanValue(default=True), group=_arg_str,
                              tooltip="Enables application of the ms gate pulse.")

        self.setattr_argument("time_ms_us_list", Scannable(
            default=[
                ExplicitScan([50.]),
            ...
            unit="us", scale=1, precision=5
                              ...
                              group=_arg_str,
                              tooltip="Pulse time for the ms gate pulse.")

        # gate parameters
        self.setattr_argument("ampls_cat_pct", PYONValue([50., 50.]), group=_arg_str,
                              tooltip="DDS amplitudes for the singlepass DDSs during the bichromatic pulses.\n"
                                      "Should be a list of [rsb_ampl_pct, bsb_ampl_pct], which are applied to [singlepass1, singlepass2].")
        self.setattr_argument("atts_cat_db", PYONValue([11., 11.]), group=_arg_str,
                              tooltip="DDS attenuations for the singlepass DDSs during the bichromatic pulses.\n"
                                      "Should be a list of [rsb_att_db, bsb_att_db], which are applied to [singlepass1, singlepass2].")
        self.setattr_argument("phases_mu_turns", PYONValue([0., 0.]), group=_arg_str,
                              tooltip="Relative phases for the singlepass DDSs during the ms gate pulse.\n"
        ...
        self.setattr_argument("freq_detuning_secular_ms_khz_list", Scannable(
            default=[
                ExplicitScan([20]),
                CenterScan(10, 4, 0.1, randomize=True),
                RangeScan(0, 10, 50, randomize=True),
            ],
            global_min=-1000, global_max=10000, global_step=1,
            unit="kHz", scale=1, precision=3
        ), group=_arg_str,
                              tooltip="")
    ...
    def prepare_experiment(self):
        ...
        (freq_center_ftw_list, freq_secular_ftw_list, time_cat2_cat_mu_list,
        ...
        self.config_experiment_list = create_experiment_config(
            # cat sweeps
            freq_center_ftw_list, freq_secular_ftw_list,
    ...
    def _prepare_experiment_cat_general(self):
        ...
        :return: tuple of (freq_center_ftw_list, freq_secular_ftw_list, time_cat2_cat_mu_list,
        ...
        # defaults - cat
        freq_center_ftw_list = array([self.qubit.frequency_to_ftw(freq_mhz * MHz)
                                          for freq_mhz in self.freq_cat_center_mhz_list])
        freq_secular_ftw_list = array([self.qubit.singlepass0.frequency_to_ftw(freq_khz * kHz)
                                           for freq_khz in self.freq_secular_khz_list])
        ...
        return (freq_center_ftw_list, freq_secular_ftw_list, time_cat2_cat_mu_list,
                phase_cat2_cat_pow_list, time_ramsey_delay_mu_list)
    ...
    def run_main(self) -> TNone:
        ...
        for trial_num in range(self.repetitions):
            for config_vals in self.config_experiment_list:
                ...
                freq_secular_ftw = int32(config_vals[1])
                ...
                self.update_profile_configuration(freq_center_ftw, freq_secular_ftw,
                                     phase_cat2_cat_pow,
                ...
                while True:
                    ...
                    if self.enable_ramsey_delay:
                        ...
                        self.qubit.on()
                        delay_mu(time_ramsey_delay_mu - 650)
                        if self.enable_dynamical_decoupling:
                            self.qubit.singlepass0_off()
                    ...
                    if self.enable_tickle_pulse:
                        ...
                        with parallel:
                            ...
                            if self.enable_dynamical_decoupling:
                                ...
                                delay_mu(self.time_tickle_dd_phase_flip_mu - 650)
                                self.qubit.singlepass0_off()
                                self.set_carrier_phase(phase_dynamical_decoupling_cat_pow,
                                                       profile=self.profile_729_cat1)
                                self.qubit.singlepass0_on()
                ...
                # store results
                self.update_results(freq_center_ftw,
                                    counts_res,
                                    freq_secular_ftw,
                                    time_cat2_cat_mu << 1,
                                    phase_cat2_cat_pow,
    ...
    def pulse_cat(self, profile: TInt32,
        ...
        if self.enable_dynamical_decoupling:
            self.set_carrier_phase(phase_dd_pow,
                                   profile=profile)
            self.qubit.singlepass0_on()
            time_pulse_mu = time_pulse_mu - 650
        else:
            self.qubit.singlepass0_off()
        self.qubit.on()
    ...
    def set_default_profile_configuration(self):
        ...
        for profile in self.profiles:
            self.ampl_beams_asf_list[profile][0] = self.ampl_qubit_asf
    ...
    @kernel(flags={'fast-math'})
    def update_profile_configuration(self, freq_center_ftw: TInt32, freq_secular_ftw: TInt32,
        ...
        :param freq_center_ftw: center frequency of the red and blue sidebands
        :param freq_secular_ftw: single-sided detuning of the red and blue sidebands from the center cat frequency
        ...
        # set up values consistent across profiles
        for profile in self.profiles:
            self.freq_beams_ftw_list[profile][0] = freq_center_ftw
            self.freq_beams_ftw_list[profile][2] = self.qubit.freq_singlepass1_default_ftw - freq_secular_ftw
            self.freq_beams_ftw_list[profile][3] = self.qubit.freq_singlepass2_default_ftw + freq_secular_ftw
