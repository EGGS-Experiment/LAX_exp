...
class IonLoadAndAramp(LAXExperiment, Experiment):
    ...
    def build_experiment(self):
        ...
        self.setattr_argument('start_east_endcap_voltage',  NumberValue(default=15., ndecimals=1, step=0.1, min=0., max=300.),
        ...
        self.setattr_argument('end_east_endcap_voltage',     NumberValue(default=205., ndecimals=1, step=0.1, min=0., max=300.),
                                                                group='Ending Trap Parameters')
        self.setattr_argument('end_west_endcap_voltage',     NumberValue(default=308., ndecimals=1, step=0.1, min=0., max=300.),
                                                                group='Ending Trap Parameters')
        self.setattr_argument('end_v_shim_voltage',          NumberValue(default=69.2, ndecimals=1, step=0.1, min=0., max=150.),
                                                                group='Ending Trap Parameters')
        self.setattr_argument('end_h_shim_voltage',          NumberValue(default=50.5, ndecimals=1, step=0.1, min=0., max=150.),
                                                                group='Ending Trap Parameters')
        self.setattr_argument('end_aramp2_voltage',          NumberValue(default=2.8, ndecimals=1, step=0.1, min=0., max=100.),
                                                                group='Ending Trap Parameters')

        self.setattr_argument('end_aramp_voltage',              NumberValue(default=23., min=3, max=30, ndecimals=0, step=0.1),
                                                                group='Aramp Parameters')
        self.setattr_argument('aramp_voltage_step',             NumberValue(default=0.25, min=0.1, max=1, ndecimals=2, step=0.01),
                                                                group='Aramp Parameters')
    ...
    def load_ion(self) -> TInt32:
        ...
        while True:
            ...
            num_ions = self.process_image(image, 'pre_aramp_orginal.png', 'pre_aramp_maipulated.png')
            self.core.break_realtime()

            # periodically check if we've reached a stop condition
            # i.e. max_time reached or termination_requested
            if idx % 5 == 0:
                if (self.core.get_rtio_counter_mu() - self.start_time) < self.time_runtime_max_mu:
    ...
    def aramp_ions(self) -> TInt32:
        ...
        for aramp_voltage in self.aramp_ions_voltage_list:
            ...
            time.sleep(2)
            self.trap_dc.set_aramp2_voltage(self.end_aramp2_voltage)
            time.sleep(2)

            image = self.camera.get_most_recent_image()
            num_ions = self.process_image(image, 'post_aramp_original.png', 'post_aramp_manipulated.png')
            if num_ions <= self.desired_num_of_ions:
                return num_ions
    ...
    @rpc
    def process_image(self, data, filepath1: TStr=None, filepath2: TStr=None) -> TInt32:
        """
        ...
        # format 1D image data array into a 2D image
        ...
        # extract number of ions in the image
        kernel1 = np.ones((2, 2))
