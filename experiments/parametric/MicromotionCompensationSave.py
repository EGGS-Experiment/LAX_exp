...
class MicromotionCompensation(ParametricSweep.ParametricSweep, Experiment):
    ...
    def prepare_experiment(self):
        ...
        # tmp remove
        self._res_holder_tmp = np.zeros((100, 2), dtype=float)
    ...
    def _generate_voltage_scan_array(self, mode_num: TInt32, iter_num: TInt32) -> TArray(TFloat, 1):
        ...
        # tmp remove
        self._res_holder_tmp = np.zeros((len(voltage_sweep_arr) * self.repetitions_per_voltage, 2), dtype=float)
        # tmp remove
    ...
    def _sweep_voltage_mode(self, mode_freq_ftw: TInt32, mode_att_mu: TInt32, mode_dc_channel_num: TInt32,
        ...
        for rep_num in range(self.repetitions_per_voltage):
            ...
            for voltage_v in voltage_scan_v_arr:
                ...
                # self._res_holder_tmp[_res_iter] = self._demodulate_counts(mode_freq_ftw, voltage_v, pmt_timestamp_list)
                self._demodulate_counts(mode_freq_ftw, voltage_v, pmt_timestamp_list, _res_iter)
                self.core.reset()
    ...
    @rpc
    def _demodulate_counts(self, freq_mu: TInt32, voltage_v: TFloat, timestamp_mu_list: TArray(TInt64, 1), iter_tmp: TInt32):
    # def _demodulate_counts(self, freq_mu: TInt32, voltage_v: TFloat, timestamp_mu_list: TArray(TInt64, 1)) -> TArray(TFloat, 1):
        ...
        # tmp remove
        self._res_holder_tmp[iter_tmp] = voltage_v
        # tmp remove
