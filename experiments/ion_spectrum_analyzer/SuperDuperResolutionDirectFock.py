...
from LAX_exp.system.subsequences import (
    ...
    SidebandReadout, QubitRAP, QubitPulseShape
)
...
class SuperDuperResolutionTMP(LAXExperiment, Experiment):
    """
    Experiment: Super Duper Resolution TMP

    Supports lots of easily configurable parameter scanning for phaser.
    Direct fock measurement.
    ...
    kernel_invariants = {
        ...
        # configs
        'profile_729_sb_readout', 'profile_729_SBC', 'profile_729_RAP', 'profile_729_PS',
    ...
    @kernel(flags={"fast-math"})
    def unb_readout_old(self, freq_ftw: TInt32, time_pipulse_mu: TInt64) -> TNone:
        # set dummy waveform before turning on
        self.qubit.set_mu(freq_ftw, asf=0x01, pow_=0x00,
                          profile=self.profile_729_sb_readout,
                          phase_mode=PHASE_MODE_CONTINUOUS)

        # set readout waveform for qubit
        self.qubit.set_profile(self.profile_729_sb_readout)
        self.qubit.cpld.io_update.pulse_mu(8)
        self.qubit.set_att_mu(self.sidebandreadout_subsequence.att_sideband_readout_mu)
        self.qubit.on()

        # do CP pulse sequence
        for phas_pow in self.phase_pow_UNB_list:
            self.qubit.set_mu(freq_ftw, asf=self.sidebandreadout_subsequence.ampl_sideband_readout_asf,
                              pow_=phas_pow,
                              profile=self.profile_729_sb_readout, phase_mode=PHASE_MODE_CONTINUOUS)
            delay_mu(time_pipulse_mu)

        # stop pulse
        self.qubit.off()

    @kernel(flags={"fast-math"})
    def unb_readout(self, time_pipulse_mu: TInt64) -> TNone:
        """
        idk
        """
        '''PRIME RAM PROFILE'''
