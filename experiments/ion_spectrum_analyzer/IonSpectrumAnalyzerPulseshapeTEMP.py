...
class IonSpectrumAnalyzer(EGGSHeating.EGGSHeating):
    ...
    """
    name = 'Ion Spectrum Analyzer Pulse Shape TEMP'
    ...
    def build_experiment(self):
        ...
        # ISA RF
        self.setattr_argument("freq_ISA_carrier_mhz_list",         Scannable(
                                                                                default=[
                                                                                    ExplicitScan([1.]),
        ...
        self.setattr_argument("freq_ISA_secular_khz_list",         Scannable(
                                                                                default=[
        ...
        self.setattr_argument("time_readout_us_list",                       Scannable(
                                                                            ...
                                                                            ), group='ISA.waveform.time_phase')
        self.setattr_argument("time_ISA_ms",                       NumberValue(default=0.01, ndecimals=5, step=1, min=0.000001, max=100000), group='ISA.waveform.time_phase')
        self.setattr_argument("phase_ISA_rsb_turns_list",          Scannable(
                                                                                default=[
        ...
        self.setattr_argument("phase_ISA_bsb_turns",               NumberValue(default=0., ndecimals=3, step=0.1, min=-1.0, max=1.0), group='ISA.waveform.time_phase')

        # ISA RF - waveform - amplitude - general
        self.setattr_argument("enable_amplitude_calibration",               BooleanValue(default=False), group='ISA.waveform.ampl')
        self.setattr_argument("att_ISA_db",                        NumberValue(default=0., ndecimals=1, step=0.5, min=0, max=31.5), group='ISA.waveform.ampl')
        self.setattr_argument("ampl_ISA_rsb_pct",                  NumberValue(default=99., ndecimals=2, step=10, min=0.0, max=99), group='ISA.waveform.ampl')
        self.setattr_argument("ampl_ISA_bsb_pct",                  NumberValue(default=0., ndecimals=2, step=10, min=0.0, max=99), group='ISA.waveform.ampl')
        # ISA RF - waveform - amplitude - dynamical decoupling - configuration
        self.setattr_argument("enable_dynamical_decoupling",                BooleanValue(default=False), group='ISA.waveform.ampl')
        self.setattr_argument("ampl_ISA_dynamical_decoupling_pct",         NumberValue(default=0., ndecimals=2, step=10, min=0.0, max=99), group='ISA.waveform.ampl')

        # ISA RF - waveform - pulse shaping
        self.setattr_argument("enable_pulse_shaping",                       BooleanValue(default=True), group='ISA.pulse_shaping')
        self.setattr_argument("type_pulse_shape",                           EnumerationValue(['sine_squared', 'error_function'], default='sine_squared'), group='ISA.pulse_shaping')
        self.setattr_argument("time_pulse_shape_rolloff_us",                NumberValue(default=50, ndecimals=1, step=100, min=10, max=100000), group='ISA.pulse_shaping')
        self.setattr_argument("freq_pulse_shape_sample_khz",                NumberValue(default=500, ndecimals=0, step=100, min=100, max=2000), group='ISA.pulse_shaping')

        # ISA RF - waveform - PSK (Phase-shift Keying)
        self.setattr_argument("enable_dd_phase_shift_keying",               BooleanValue(default=False), group='ISA.waveform.psk')
        self.setattr_argument("num_dynamical_decoupling_phase_shifts",      NumberValue(default=3, ndecimals=0, step=10, min=1, max=100), group='ISA.waveform.psk')
    ...
    def prepare_experiment(self):
        ...
        # total_phaser_channel_amplitude =                                    (self.ampl_ISA_rsb_pct +
        #                                                                      self.ampl_ISA_bsb_pct +
        #                                                                      self.ampl_ISA_dynamical_decoupling_pct)
        ...
        '''ISA HEATING - TIMING'''
        self.time_ISA_mu =                             self.core.seconds_to_mu(self.time_ISA_ms * ms)

        # ensure ISA heating time is a multiple of the phaser sample period
        # note: 1 frame period = 4 ns/clock * 8 clock cycles * 10 words = 320ns
        # todo: move to an internal phaser function
        if self.time_ISA_mu % self.phaser_eggs.t_sample_mu:
            # round ISA heating time up to the nearest multiple of phaser frame period
            t_sample_multiples =                                round(self.time_ISA_mu / self.phaser_eggs.t_sample_mu + 0.5)
        ...
        self.freq_ISA_carrier_hz_list =                        np.array(list(self.freq_ISA_carrier_mhz_list)) * MHz
        self.freq_ISA_secular_hz_list =                        np.array(list(self.freq_ISA_secular_khz_list)) * kHz
        self.phase_ISA_rsb_turns_list =                np.array(list(self.phase_ISA_rsb_turns_list))

        # map phase to index to facilitate pulse shaping
        self.phase_ISA_index_to_turns =                np.arange(len(self.phase_ISA_rsb_turns_list))
        # create data structures to allow programmatic recording & playback of DMA handles
        self.phase_ISA_dma_names_rise =                ['_ISA_rise_' + str(i) for i in range(len(self.phase_ISA_rsb_turns_list))]
        self.phase_ISA_dma_names_fall =                ['_ISA_fall_' + str(i) for i in range(len(self.phase_ISA_rsb_turns_list))]
        self.phase_ISA_dma_handles_rise =              [(0, np.int64(0), np.int32(0))] * len(self.phase_ISA_rsb_turns_list)
        self.phase_ISA_dma_handles_fall =              [(0, np.int64(0), np.int32(0))] * len(self.phase_ISA_rsb_turns_list)


        # implement frequency sub-repetitions by "multiplying" the ISA frequency
        self.freq_ISA_carrier_hz_list =                        np.repeat(self.freq_ISA_carrier_hz_list, self.sub_repetitions)
        ...
        self.config_ISA_list =                         np.zeros((len(self.freq_sideband_readout_ftw_list) *
                                                                          ...
                                                                          len(self.freq_ISA_carrier_hz_list) *
        ...
        self.config_ISA_list[:, [1, 2, -3, -2, -1, 0]] =   np.stack(np.meshgrid(self.freq_ISA_carrier_hz_list,
                                                                                     self.freq_ISA_secular_hz_list,
                                                                                     self.phase_ISA_index_to_turns,
        ...
        if self.randomize_config:                               np.random.shuffle(self.config_ISA_list)


        '''ISA HEATING - AMPLITUDE CALIBRATION'''
        ...
        # calculate calibrated ISA sidebands amplitudes
        if self.enable_amplitude_calibration:
            for i, (_, carrier_freq_hz, secular_freq_hz, _, _, _, _, _) in enumerate(self.config_ISA_list):
                ...
                # adjust sideband amplitudes to have equal power and normalize to ampl_ISA_frac
                ...
                #                                                              ((self.ampl_ISA_pct / 100.) / (transmitted_power_frac[0] + transmitted_power_frac[1])))
                scaled_power_pct =                              (np.array([transmitted_power_frac[1], transmitted_power_frac[0]]) *
                                                                 ((self.ampl_ISA_rsb_pct / 100.) / (transmitted_power_frac[0] + transmitted_power_frac[1])))
        ...
        '''ISA HEATING - ISA RF CONFIGURATION'''
    ...
    def _prepare_pulseshape(self):
        ...
        ampl_dd_pct =                               self.ampl_ISA_dynamical_decoupling_pct
        if not self.enable_dynamical_decoupling:    ampl_dd_pct = 0.

        # create holder object for pulse amplitudes
        self.ampl_pulse_shape_frac_list =           np.tile(np.array([self.ampl_ISA_rsb_pct,
                                                                      self.ampl_ISA_bsb_pct,
                                                                      ampl_dd_pct]) / 100.,
    ...
    def _prepare_psk(self):
        ...
        # divide total ISA heating time into PSK segments
        self.time_psk_delay_mu =                                            np.int64(round(self.time_ISA_mu / (self.num_dynamical_decoupling_phase_shifts + 1)))
    ...
    def run_main(self):
        ...
        _handle_ISA_pulseshape_rise =      self.core_dma.get_handle('_PHASER_PULSESHAPE_RISE')
        _handle_ISA_pulseshape_fall =      self.core_dma.get_handle('_PHASER_PULSESHAPE_FALL')
        self.core.break_realtime()

        # get phaser pulse shaping sequence handles for each RSB
        for i in range(len(self.phase_ISA_rsb_turns_list)):
            self.phase_ISA_dma_handles_rise[i] = self.core_dma.get_handle(self.phase_ISA_dma_names_rise[i])
            self.core.break_realtime()
            self.phase_ISA_dma_handles_fall[i] = self.core_dma.get_handle(self.phase_ISA_dma_names_fall[i])
            self.core.break_realtime()
        ...
        at_mu(self.phaser_eggs.get_next_frame_mu())
        self.phaser_eggs.channel[0].set_att(self.att_ISA_db * dB)
        delay_mu(self.phaser_eggs.t_sample_mu)
        self.phaser_eggs.channel[1].set_att(self.att_ISA_db * dB)
        self.core.break_realtime()
        # tmp remove
        ...
        for trial_num in range(self.repetitions):
            ...
            for config_vals in self.config_ISA_list:
                ...
                # configure ISA tones and set readout frequency
                ...
                self.core_dma.playback_handle(self.phase_ISA_dma_handles_rise[phase_rsb_index])

                # ISA - RUN
                self.phaser_run(ampl_rsb_frac, ampl_bsb_frac, ampl_dd_frac)

                # ISA - STOP
                self.core_dma.playback_handle(self.phase_ISA_dma_handles_fall[phase_rsb_index])
    ...
    def phaser_setup(self) -> TNone:
        ...
        """
        # get starting phase values for pulse shaping
        carrier_freq_hz =   self.config_ISA_list[0, 1]
        sideband_freq_hz =  self.config_ISA_list[0, 2]
        offset_freq_hz = self.config_ISA_list[0,8]
        self.core.break_realtime()

        # record DMA sequences for each RSB phase
        for i in range(len(self.phase_ISA_rsb_turns_list)):

            # get RSB phase
            phase_rsb_turns = self.phase_ISA_rsb_turns_list[i]

            # configure ISA tones and set readout frequency; also necessary to ensure phase delays are correctly set
            self.phaser_configure(carrier_freq_hz, sideband_freq_hz, offset_freq_hz, phase_rsb_turns)
            self.core.break_realtime()
            ...
            if self.enable_pulse_shaping:
                with self.core_dma.record(self.phase_ISA_dma_names_rise[i]):
            ...
            if self.enable_pulse_shaping:
                with self.core_dma.record(self.phase_ISA_dma_names_fall[i]):
                    # set amplitude values at given time
    ...
    @kernel(flags={"fast-math"})
    def phaser_configure(self, carrier_freq_hz: TFloat, sideband_freq_hz: TFloat, offset_freq_hz: TFloat, phase_rsb_turns: TFloat) -> TNone:
        """
        Configure the tones on phaser for ISA.
        ...
        # oscillator 1 (BSB)
        self.phase_phaser_turns_arr[0, 1] = ((sideband_freq_hz + offset_freq_hz)* self.phaser_eggs.t_sample_mu * ns) + self.phase_ISA_bsb_turns
        self.phase_phaser_turns_arr[1, 1] = ((sideband_freq_hz + offset_freq_hz) * self.phaser_eggs.t_sample_mu * ns) + self.phase_ISA_bsb_turns
    ...
    def phaser_pulseshape_point(self, ampl_rsb_frac: TFloat, ampl_bsb_frac: TFloat, ampl_dd_frac: TFloat) -> TNone:
        """
        Convenience function for pulse-shaping the ISA waveform.
    ...
    def phaser_run_nopsk(self, ampl_rsb_frac: TFloat, ampl_bsb_frac: TFloat, ampl_dd_frac: TFloat) -> TNone:
        """
        Activate phaser channel outputs for ISA heating.
        Sets the same oscillator amplitudes for both channels.
        Arguments:
            ampl_rsb_frac   (float) : the red sideband amplitude (as a decimal fraction).
            ampl_bsb_frac   (float) : the blue sideband amplitude (as a decimal fraction).
    ...
    def analyze(self):
        ...
        try:
            ...
            if sorting_col_num == 3:
                ...
                self.set_dataset('temp.ISAheating.results', res_dj, broadcast=True, persist=False, archive=False)
                self.set_dataset('temp.ISAheating.rid', self.scheduler.rid, broadcast=True, persist=False, archive=False)
