...
class EGGSHeatingRDXGlobalOffset(LAXExperiment, Experiment):
    """
    Experiment: EGGS Heating Global Offset
    ...
    Adds a global offset to remove leakage at carrier.
    """
    name = 'EGGS Heating RDX Global Offset'
    kernel_invariants = {
    ...
    def build_experiment(self):
        ...
        self.setattr_argument("freq_eggs_heating_offset_mhz",               NumberValue(default=6., precision=3, step=1, min=-10., max=10.), group='EGGS_Heating.frequencies')
        ...
        self.setattr_argument("att_eggs_heating_db",            NumberValue(default=3., precision=1, step=0.5, min=0, max=31.5), group='EGGS_Heating.waveform.ampl')
        ...
        self.setattr_argument("ampl_eggs_heating_carrier_pct",  NumberValue(default=19., precision=2, step=10, min=0.0, max=99), group='EGGS_Heating.waveform.ampl')
        ...
        self.setattr_argument("freq_pulse_shape_sample_khz",    NumberValue(default=1000, precision=0, step=100, min=100, max=2000), group='EGGS_Heating.pulse_shaping')
    ...
    def prepare_experiment(self):
        ...
        # tmp remove
        self.freq_eggs_heating_offset_hz = self.freq_eggs_heating_offset_mhz * MHz
        # tmp remove
    ...
    def _prepare_waveform(self) -> TNone:
        ...
        th0 = self.freq_eggs_heating_offset_hz * (self.phaser_eggs.t_sample_mu * ns)
        _sequence_blocks[:, 1, 1] = self.phase_eggs_heating_bsb_turns + phase_bsb_update_delay_turns + th0
        _sequence_blocks[:, 2, 1] = 2. * th0
        # tmp remove
    ...
    def run_main(self) -> TNone:
        ...
        for trial_num in range(self.repetitions):
            ...
            while _config_iter < self.num_configs:
                ...
                self.phaser_eggs.frequency_configure(carrier_freq_hz - self.freq_eggs_heating_offset_hz,
                                                     [
                                                         -sideband_freq_hz + self.freq_eggs_heating_offset_hz,
                                                         sideband_freq_hz + self.freq_eggs_heating_offset_hz,
                                                         self.freq_eggs_heating_offset_hz,
                                                         0., 0.
                                                     ],
                                                     phase_ch1_turns)
                self.core.break_realtime()
    ...
    def phaser_configure(self, carrier_freq_hz: TFloat, sideband_freq_hz: TFloat, phase_ch1_offset_turns: TFloat) -> TNone:
        ...
        phase_ch1_turns = phase_ch1_offset_turns + ((carrier_freq_hz + self.freq_eggs_heating_offset_hz) * self.phaser_eggs.time_latency_ch1_system_ns * ns)
        ...
        with parallel:
            self.phaser_eggs.channel[0].oscillator[0].set_frequency(-sideband_freq_hz + self.freq_eggs_heating_offset_hz)
            self.phaser_eggs.channel[1].oscillator[0].set_frequency(-sideband_freq_hz + self.freq_eggs_heating_offset_hz)
            delay_mu(self.phaser_eggs.t_sample_mu)
        # set oscillator 1 (BSB)
        with parallel:
            self.phaser_eggs.channel[0].oscillator[1].set_frequency(sideband_freq_hz + self.freq_eggs_heating_offset_hz)
            self.phaser_eggs.channel[1].oscillator[1].set_frequency(sideband_freq_hz + self.freq_eggs_heating_offset_hz)
            delay_mu(self.phaser_eggs.t_sample_mu)
        # set oscillator 2 (carrier)
        with parallel:
            self.phaser_eggs.channel[0].oscillator[2].set_frequency(0. + self.freq_eggs_heating_offset_hz)
            self.phaser_eggs.channel[1].oscillator[2].set_frequency(0. + self.freq_eggs_heating_offset_hz)
            delay_mu(self.phaser_eggs.t_sample_mu)
