...
_DMA_HANDLE_RESET = "qubit_repump_depletion_reset"
_DMA_HANDLE_READOUT = "qubit_repump_depletion_readout"


class QubitRepumpDepletionSD(EnvExperiment):
    """
    Qubit Repump Depletion Sweep
    Measures ion fluorescence vs 854nm pulse time and frequency.
    """
    ...
    def build(self):
        ...
        # qubit parameters
        self.setattr_argument("time_pipulse_us",                        NumberValue(default=400, ndecimals=5, step=1, min=1, max=10000))
        self.setattr_argument("time_qubit_repump_initialize_us",        NumberValue(default=100, ndecimals=5, step=1, min=1, max=1000000))
        self.setattr_argument("time_qubit_repump_us_list",              Scannable(default=
    ...
    def prepare(self):
        ...
        self.time_repump_qubit_mu =                                     self.core.seconds_to_mu(self.time_qubit_repump_initialize_us * us)
        ...
        # depletion timing
        self.time_qubit_repump_mu_list =                                [self.core.seconds_to_mu(time_us * us) for time_us in self.time_qubit_repump_us_list]
        max_time_us =                                                   np.max(list(self.time_qubit_repump_mu_list))
        self.time_delay_mu_list =                                       [self.core.seconds_to_mu((max_time_us - time_us) * us) for time_us in self.time_qubit_repump_us_list]
        self.num_time_points_list =                                     list(range(len(self.time_qubit_repump_us_list)))
        self.time_qubit_ =                                          self.core.seconds_to_mu(self.time_pipulse_us * us)
        ...
        # set up datasets
        self.set_dataset("qubit_repump_depletion", [])
        self.setattr_dataset("qubit_repump_depletion")
        self.set_dataset("qubit_repump_depletion_processed", np.zeros([len(self.time_rabi_mu_list), 2]))
        self.setattr_dataset("qubit_repump_depletion_processed")
    ...
    def run(self):
        ...
        for trial_num in range(self.repetitions):
            ...
            for i in self.num_time_points_list:
                ...
                time_qubit_repump_mu = self.time_qubit_repump_mu_list[i]
                ...
                # qubit repump depletion w/qubit laser
                self.dds_board.cfg_switches(0b1100)
                delay_mu(time_qubit_repump_mu)
                self.dds_qubit.cfg_sw(0b0100)
                ...
                # update dataset
                with parallel:
                    self.update_dataset(time_qubit_repump_mu, self.pmt_counter.fetch_count())
                    self.core.break_realtime()
    ...
    def update_dataset(self, time_mu, pmt_counts):
        ...
        """
        self.append_to_dataset('qubit_repump_depletion', [self.core.mu_to_seconds(time_mu), pmt_counts])
    ...
    def analyze(self):
        ...
        self.qubit_repump_depletion = np.array(self.qubit_repump_depletion)

        # get sorted x-values (time, seconds)
        time_list_s = sorted(set(self.qubit_repump_depletion[:, 0]))
        ...
        for time_s, pmt_counts in self.qubit_repump_depletion:
