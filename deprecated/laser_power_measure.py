...
# todo: account for beam saturation
# todo: implement running average
# todo: set amplitude/freq in profile 0, then clean up by resetting to default profile
...
class LaserPowerMeasure(EnvExperiment):
    ...
    def build(self):
        ...
        # photodiode
        self.setattr_argument("laser_wavelength_nm",            NumberValue(default=397, precision=0, step=10, min=200, max=3200), group='photodiode')
    ...
    @rpc(flags={"async"})
    def update_dataset(self, i: TInt32, volts_mu_arr):
        """
    ...
    def analyze(self):
        ...
        voltage_stdev_v =   np.std(self.results, axis=0)
        voltage_stderr_v =  voltage_stdev_v / np.sqrt(len(self.results))
        ...
        power_std_uw = voltage_stdev_v / (self.photodiode_termination_ohms * self.photodiode_responsivity_value) * 1e6

        # store results in dataset
        self.set_dataset('voltage_mean_v', voltage_mean_v)
        self.set_dataset('voltage_stdev_v', voltage_stdev_v)
