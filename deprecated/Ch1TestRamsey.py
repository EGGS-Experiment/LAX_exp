...
class Ch1Ramsey(LAXExperiment, Experiment):
    """
    Experiment: Ch1 Ramsey
    ...
    via sideband thermometry or RAP.
    """
    name = 'Ch1 Ramsey'
    kernel_invariants = {
        # hardware parameters
        'att_heating_mu', 'freq_secular_hz_list', 'phase_heating_rsb_turns_list',
        'phase_heating_ch1_turns_list', 'waveform_index_to_phase_rsb_turns', 'num_configs',
    ...
    def build_experiment(self):
        ...
        self.setattr_argument("freq_heating_carrier_mhz_list", Scannable(
                                                                    ...
                                                                    ), group='Heating.frequencies')
        self.setattr_argument("freq_heating_secular_khz_list", Scannable(
                                                                        default=[
                                                                            # ExplicitScan([1303]),
        ...
        # RF - waveform - timing & phase
        self.setattr_argument("time_heating_us", NumberValue(default=1000, precision=2, step=500, min=0.04, max=100000000), group='Heating.waveform.time_phase')
        self.setattr_argument("phase_heating_rsb_turns_list", Scannable(
                                                                        default=[
                                                                            ExplicitScan([0.]),
                                                                    ...
                                                                    ), group='Heating.waveform.time_phase')
        self.setattr_argument("phase_heating_ch1_turns_list", Scannable(
                                                                        default=[
                                                                            ExplicitScan([0.4668]),
        ...
        self.setattr_argument("phase_heating_bsb_turns", NumberValue(default=0., precision=3, step=0.1, min=-1.0, max=1.0), group='Heating.waveform.time_phase')

        # RF - waveform - amplitude - general
        self.setattr_argument("att_heating_db",        NumberValue(default=22., precision=1, step=0.5, min=0, max=31.5), group='Heating.waveform.ampl')
        self.setattr_argument("ampl_heating_rsb_pct",  NumberValue(default=40., precision=2, step=10, min=0.0, max=99), group='Heating.waveform.ampl')
        self.setattr_argument("ampl_heating_bsb_pct",  NumberValue(default=40., precision=2, step=10, min=0.0, max=99), group='Heating.waveform.ampl')
        self.setattr_argument("ampl_heating_carrier_pct",  NumberValue(default=.2, precision=2, step=10, min=0.0, max=99), group='Heating.waveform.ampl')

        # RF - waveform - pulse shaping
        self.setattr_argument("enable_pulse_shaping",           BooleanValue(default=False), group='Heating.pulse_shaping')
        self.setattr_argument("type_pulse_shape",               EnumerationValue(['sine_squared', 'error_function', 'slepian'], default='sine_squared'), group='Heating.pulse_shaping')
        self.setattr_argument("time_pulse_shape_rolloff_us",    NumberValue(default=100, precision=1, step=100, min=0.2, max=100000), group='Heating.pulse_shaping')
        self.setattr_argument("freq_pulse_shape_sample_khz",    NumberValue(default=1000, precision=0, step=100, min=100, max=5000), group='Heating.pulse_shaping')
        ...
        self.pulse_shaper = PhaserPulseShaper2(self, np.array([0., 0., 0.5, 0., 0.])) # phase delays set field geometries (0.5 for osc_2 => dipole)
    ...
    def prepare_experiment(self):
        ...
        freq_carrier_hz_list = np.array(list(self.heating_carrier_mhz_list)) * MHz
        self.freq_secular_hz_list = np.array(list(self.freq_heating_secular_khz_list)) * kHz
        self.phase_heating_rsb_turns_list = np.array(list(self.phase_heating_rsb_turns_list))
        self.phase_heating_ch1_turns_list = np.array(list(self.phase_heating_ch1_turns_list))

        # map phase to index to facilitate waveform recording
        self.waveform_index_to_phase_rsb_turns = np.arange(len(self.phase_heating_rsb_turns_list))
        ...
        self.config_experiment_list = np.zeros((
            ...
            len(self.freq_secular_hz_list) *
            len(self.phase_heating_rsb_turns_list) *
            len(self.phase_heating_ch1_turns_list) *
        ...
        self.config_experiment_list[:, [1, 2, -3, -2, -1, 0]] = np.stack(np.meshgrid(
            ...
            self.freq_secular_hz_list,
            self.waveform_index_to_phase_rsb_turns,
            self.phase_heating_ch1_turns_list,
    ...
    def _prepare_waveform(self) -> TNone:
        ...
        self.waveform_index_to_pulseshaper_id = np.zeros(len(self.phase_heating_rsb_turns_list),
        ...
        _osc_vals = np.zeros((num_blocks, 3, 2), dtype=float)

        # set oscillator amplitudes
        _osc_vals[:, 0, 0] = self.ampl_heating_rsb_pct
        _osc_vals[:, 1, 0] = self.ampl_heating_bsb_pct
        _osc_vals[:, 2, 0] = self.ampl_heating_carrier_pct
        ...
        t_update_delay_s_list = (self.core.mu_to_seconds(self.phaser_eggs.t_sample_mu)) * np.arange(3)
        ...
        for i, phase_rsb in enumerate(self.phase_heating_rsb_turns_list):
            # create local copy of _sequence_blocks
            # note: no need to deep copy b/c it's filled w/immutables
            _osc_vals_local = np.copy(_osc_vals)

            # update sequence block with rsb phase
            _osc_vals_local[:, 0, 1] += phase_rsb

            _osc_vals_local_ch0_on = _osc_vals_local
            _osc_vals_local_ch0_on[0, 0, 0] = 0.
            _osc_vals_ch0_local_on[0, 1, 0] = 0.

            _osc_vals_local_ch0_off = _osc_vals_local
            _osc_vals_local_ch0_off[1, 0, 0] = 0.
            _osc_vals_local_ch0_off[1, 1, 0] = 0.

            _sequence_blocks_local_ch0_on = [
                {
                    "oscillator_parameters": _osc_vals_local_ch0_on[i],
                    "config": {
                        "time_us": self.time_heatings_us / num_blocks,
            ...
            _sequence_blocks_local_ch0_off = [
                {
                    "oscillator_parameters": _osc_vals_local_ch0_off[i],
                    "config": {
                        "time_us": self.time_heatings_us / num_blocks,
                        "pulse_shaping": self.enable_pulse_shaping and ((i == 0) or (i == num_blocks - 1)),
                        "pulse_shaping_config": {
            ...
            self.spinecho_wizard.sequence_blocks = _sequence_blocks_local_ch0_on
            ...
            self.spinecho_wizard.sequence_blocks = _sequence_blocks_local_ch0_off
            self.spinecho_wizard.compile_waveform()
    ...
    def run_main(self) -> TNone:
        ...
        for trial_num in range(self.repetitions):
            ...
            while _config_iter < self.num_configs:
                ...
                phase_rsb_turns = self.phase_heating_rsb_turns_list[phase_rsb_index]
