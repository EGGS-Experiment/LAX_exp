...
class TemperatureMeasurementCalibration(EnvExperiment):
    ...
    def build(self):
        ...
        self.setattr_argument("dds_freq_mhz_list",                          Scannable(
                                                                                default=RangeScan(95, 125, 10, randomize=True),
        ...
        self.setattr_argument("dds_ampl_min_pct",                           NumberValue(default=0.1, ndecimals=2, step=1, min=5, max=40))
        ...
        # search parameters
        self.setattr_argument("target_voltage_mv",                          NumberValue(default=46, ndecimals=3, step=1, min=-10000, max=10000))
    ...
    def _recursion_search(self, freq_ftw, ampl_min_asf, ampl_max_asf):
        ...
        # # tmp remove
        # self.core.break_realtime()
        # print('\trecursion called: ', ampl_tmp_asf / 0x3FFF * 100, 2)
        # self.core.break_realtime()
        # # tmp remove
    ...
    def analyze(self):
        ...
        # tmp remove
        initialize_time = '\tinitialize time: {:.2f}'.format(datetime.timestamp(datetime.now()) - self.time_start)
        print()
        print('\t\t\tFINAL RES: ', initialize_time)
        print()
        print(np.array([calib_freq_mhz, calib_ampl_frac]).transpose())
        print()
        print(calib_ampl_frac)
        # tmp remove
