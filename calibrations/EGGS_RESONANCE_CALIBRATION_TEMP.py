...
class EGGSVoltagesCalibration(LAXExperiment, EnvExperiment):
    ...
    def build_experiment(self):
        ...
        self.setattr_argument("time_readout_us", NumberValue(
            default=134., min=1, max=100000, step=1,
            unit="us", scale=1, ndecimals=5), group=self.name)
        ...
        self.setattr_argument("freq_eggs_heating_carrier_mhz_list", Scannable(
            ...
            global_min=30, global_max=400, global_step=1, unit="MHz", scale=1, ndecimals=6
        ), group='EGGS_Heating.frequencies')

        self.setattr_argument("freq_eggs_heating_secular_one_sb_khz", NumberValue(
            default=1408.00, min=0, max=10000, step=1,
    ...
    def prepare_experiment(self):
        ...
        self.freq_eggs_secular_one_sb_hz_list = np.array(list([self.freq_eggs_heating_secular_one_sb_khz])) * kHz
        ...
        self.config_eggs_heating_list = np.zeros((len(self.freq_sideband_readout_ftw_list) *
                                                  ...
                                                  len(self.freq_eggs_secular_one_sb_hz_list) *
        ...
        self.config_eggs_heating_list[:, [1, 2, 3,-2, -1, 0]] = np.stack(np.meshgrid(self.freq_eggs_carrier_hz_list,
                                                                                   self.freq_eggs_secular_two_sb_hz_list,
                                                                                    self.freq_eggs_secular_one_sb_hz_list,
    ...
    def run_main(self):
        ...
        for trial_num in range(self.repetitions):
            ...
            for config_vals in self.config_eggs_heating_list:
                ...
                two_sb_secular_freq_hz = config_vals[2]
                one_sb_secular_freq_hz = config_vals[3]
                ampl_rsb_frac = config_vals[4]
                ampl_bsb_frac = config_vals[5]
                ampl_dd_frac = config_vals[6]
                phase_rsb_turns = config_vals[7]
                time_readout_mu = np.int64(config_vals[8])
                self.core.break_realtime()

                self._run_eggs(carrier_freq_hz, one_sb_secular_freq_hz, phase_rsb_turns, freq_readout_ftw,
                               0., ampl_bsb_frac, ampl_dd_frac, time_readout_mu)

                self._run_eggs(carrier_freq_hz, two_sb_secular_freq_hz, phase_rsb_turns, freq_readout_ftw,
