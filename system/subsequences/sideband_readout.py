...
class SidebandReadout(LAXSubsequence):
    ...
    def build_subsequence(self):
        ...
        self.setattr_argument("freq_rsb_readout_mhz_list",      Scannable(
                                                                    default=[
                                                                        ExplicitScan([101.4547]),
                                                                        CenterScan(101.4205, 0.01, 0.00025, randomize=True),
                                                                    ],
        ...
        self.setattr_argument("freq_bsb_readout_mhz_list",      Scannable(
                                                                    default=[
                                                                        ExplicitScan([102.7495]),
                                                                        CenterScan(102.8368, 0.01, 0.00025, randomize=True),
                                                                    ],
                                                                    global_min=30, global_max=200, global_step=1,
                                                                    unit="MHz", scale=1, ndecimals=5
                                                                ), group=self.name)
        ...
        self.setattr_argument("time_sideband_readout_us",       NumberValue(default=140.5, ndecimals=5, step=1, min=1, max=10000), group=self.name)
