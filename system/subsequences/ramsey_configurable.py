...
class RamseyConfigurable(LAXSubsequence):
    """
    Subsequence: Ramsey (Configurable)
    ...
    def build_subsequence(self):
        self.setattr_argument('att_ramsey_db',             NumberValue(default=31.5, ndecimals=1, step=0.5, min=0, max=31.5), group=self.name)
    ...
    def prepare_subsequence(self):
        ...
        self.att_ramsey_mu =            att_to_mu(self.att_ramsey_db * dB)
    ...
    def squeeze(self):
        ...
        self.qubit.write32(_AD9910_REG_CFR1, (1 << 16))
    ...
    def configure(self, freq_ftw: TInt32, phase_pow: TInt32, time_mu: TInt64) -> TInt64:
        ...
        time_half_period_ns =                               1. / (2. * ns * self.qubit.ftw_to_frequency(freq_ftw))
        ...
        phase_antisqueeze_turns =                           0.5 + self.qubit.pow_to_turns(phase_pow)
        phase_antisqueeze_pow =                             self.qubit.turns_to_pow(phase_antisqueeze_turns)
        ...
        self.qubit.set_mu(freq_ftw, asf=self.qubit.ampl_qubit_asf, profile=1,
                                   pow_=phase_antisqueeze_pow, phase_mode=PHASE_MODE_CONTINUOUS)
        # blank waveform
        self.qubit.set_mu(freq_ftw, asf=0x0, profile=2,
                                   pow_=0x0, phase_mode=PHASE_MODE_CONTINUOUS)
